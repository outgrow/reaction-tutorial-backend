...
export default async function catalogItems(_, args, context) {
  const { shopIds: opaqueShopIds, tagIds: opaqueTagIds, filterTagIds: opaqueFilterTagIds, ...connectionArgs } = args;
  ...
  const filterTagIds = opaqueFilterTagIds && opaqueFilterTagIds.map(decodeTagOpaqueId);
